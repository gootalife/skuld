...
def getGitDiff(commitHash, fileName):
    ...
    for index, line in enumerate(diff):
        # 最初のステータス表示を無視
        if index <= 3 or line.startswith('@@') or line.startswith('-') or line.startswith('++') or line.startswith('--'):
            continue;
        if line.startswith('+'):
            line = line.replace('+', '', 1)
        else:
        ...
        aline = line.lstrip()
        # コメント行を無視
        if aline.startswith('//') or aline.startswith('/*') or aline.startswith('*') or aline.startswith('*/') or aline.startswith('*/'):
            continue;
        code.append(line)
...
# 各コミットハッシュの変更の取得
def getGitDiffs(commitLogs, currentDirectory, projectDirectory, extensions, projectName):
    for index, item in commitLogs.iterrows():
        ...
        for fileName in fileList:
            ...
            if ext in extensions:  # 拡張子が指定のものだけ
        ...
        os.chdir(currentDirectory) # カレントディレクトリへ移動
        # 出力
        with open('data/projects/{0}/logs/commits/{1}.txt'.format(projectName, commitHash), 'w', encoding='utf-8') as file:
            file.write(code)
...
if __name__ == '__main__':
    ...
    # git diffの複数取得
    getGitDiffs(commitLogs, currentDirectory, projectDirectory, extensions, projectName)
